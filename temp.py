...
def format_notification_for_waybar(notifications):
    response_struct = {
        "text": f"{len(notifications)}",
        "alt": f"{len(notifications) if len(notifications) <= 9 else '9+'}",
        "tooltip": "\r".join([f"{i+1}: {m['summary']}" for i, m in enumerate(notifications)]),
...
def on_notifications_changed(iface, prop_changed, prop_invalidated):
    if 'Notifications' in prop_invalidated:
        format_notification_for_waybar(mako_object.Notifications)
